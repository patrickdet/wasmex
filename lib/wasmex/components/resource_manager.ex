defmodule Wasmex.Components.ResourceManager do
  ...
  defmodule State do
    defstruct [
      :resources,       # Map of resource_id -> {pid, store_id}
      :next_id,         # Next available resource ID
      :store_resources, # Map of store_id -> MapSet of resource_ids
      :monitors        # Map of monitor_ref -> resource_id
    ]
  end
        ...
        {:ok, pid} ->
          ...
          case create_native_handle(store, resource_id, type_name) do
            {:ok, handle} ->
              ...
              Logger.debug("Created process resource #{resource_id} (pid: #{inspect(pid)}) of type #{type_name} for store #{store_id}")
  ...
  def handle_info({:DOWN, monitor_ref, :process, pid, reason}, state) do
    ...
    case Map.get(state.monitors, monitor_ref) do
      resource_id when is_integer(resource_id) ->
        Logger.debug("Resource process #{resource_id} (pid: #{inspect(pid)}) terminated: #{inspect(reason)}")
