defmodule Wasmex.Components.WasiIntegrationTest do
  ...
  describe "WASI filesystem operations" do
    ...
    test "can delete files", %{instance: instance} do
      ...
      assert_receive {:returned_function_call, {:ok, result}, ^from}, 5000
      case result do
        :ok -> :ok  # Unit type result is encoded as :ok
        {:ok, _} -> :ok
    ...
    test "filesystem is isolated to preopened directory", %{instance: instance, test_dir: test_dir} do
      ...
      case result do
        {:error, error} -> 
          assert error =~ "Failed" or error =~ "denied" or error =~ "not permitted" or error =~ "preopened"
        {:ok, _} -> 
  ...
  describe "WASI clock operations" do
    ...
    test "can get current time", %{instance: instance} do
      ...
      assert nanos2 >= nanos  # Should be greater or equal (might be same on fast systems)
    end
