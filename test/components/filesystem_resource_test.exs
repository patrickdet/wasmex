defmodule Wasmex.Components.FilesystemResourceTest do
  ...
  describe "filesystem resources" do
    setup do
      ...
      unless File.exists?(@filesystem_component_path) do
        build_cmd = "cd test/component_fixtures/filesystem-component && cargo component build --release"
    ...
    test "file handle resource lifecycle", %{instance: instance} do
      ...
      # Create a directory
      :ok = Components.Instance.call_function(instance, ["test:filesystem/types", "create-test-directory"], [], from)
      ...
      files = for i <- 1..3 do
        filename = "file#{i}.txt"
        :ok = Components.Instance.call_function(
          instance,
    ...
    test "concurrent file handle operations", %{instance: instance} do
      ...
      assert_receive {:returned_function_call, {:ok, {:ok, file_handle}}, ^from}, 5000,
        "Timeout creating file"
      
      # Simulate concurrent writes (not truly concurrent but sequential)
      writes = for i <- 1..10 do
        data = "Line #{i}\n"
        bytes = :erlang.binary_to_list(data)
      ...
      assert Enum.all?(writes, fn {:ok, _} -> true; _ -> false end)
    end
    ...
    test "directory operations", %{instance: instance} do
      ...
      for name <- filenames do
        ...
        assert_receive {:returned_function_call, {:ok, {:ok, _}}, ^from}, 5000,
      ...
      assert_receive {:returned_function_call, {:ok, file_list}, ^from}, 5000,
    ...
    test "file position tracking", %{instance: instance} do
      ...
      assert_receive {:returned_function_call, {:ok, {:ok, file}}, ^from}, 5000,
        "Timeout creating file"
      
      # Write data in chunks
      chunks = ["First chunk. ", "Second chunk. ", "Third chunk."]
      
      written_sizes = for chunk <- chunks do
        ...
        assert_receive {:returned_function_call, {:ok, {:ok, written}}, ^from}, 5000,
  ...
  describe "resource lifecycle stress testing" do
    ...
    test "cross-store resource isolation", %{component_path: component_path} do
      ...
      stores_and_resources = for i <- 1..3 do
        wasi_options = %WasiP2Options{
          args: [],
          env: %{},
          inherit_stdin: true,
