defmodule Wasmex.Components.HostResourceTest do
  ...
  describe "host resource creation" do
    ...
    test "fails to create resource without type registration", %{store: store} do
      ...
      case result do
        {:error, _} -> assert true
        nil -> assert true
        _ -> flunk("Expected error when creating resource without type registration, got: #{inspect(result)}")
      end
    end
  ...
  describe "host resource method calls" do
    ...
    test "calls method on host resource", %{store: store, resource: resource} do
      ...
      result = Wasmex.Native.host_resource_call_method(
        store.resource,
        resource,
        "test-method",
        []
    ...
    test "resources are cleaned up when store is destroyed" do
      resource = 
        fn ->
          ...
          resource = Wasmex.Native.host_resource_new(store.resource, resource_id, "temp-resource")
  ...
  describe "type conversion" do
    ...
    test "converts basic types to Val", %{store: store, resource: resource} do
      ...
      test_values = [
        ...
        {:some, 42},    # Option::Some
        :none,          # Option::None
        {:ok, "success"}, # Result::Ok
        {:error, "fail"}  # Result::Err
      ]
